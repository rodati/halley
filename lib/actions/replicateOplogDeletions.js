...
async function replicateOplogDeletions (rawSpecs, pgPool, mongoConnection, concurrency, deleteSingle) {
  winston.info(`[oplog] Searching for last sync date ...`)
  const pgClient = await pgPool.connect()
  const specs = _.values(rawSpecs)
  const selectMax = []
    ...
    if (collection) {
      ...
      if (collection.values.length === 1000) {
        ...
        try {
          ...
          collection.values = undefined
        } catch (error) {
          winston.warn('[replicateOplogDeletions] Error with bulk delete. Attempting single docs deletes')
          try {
            const { spec, values } = collection
            for (const currentDeleteId of values) {
              await deleteSingle(spec, pgPool, currentDeleteId)
            }
          } catch (err) {
...
async function deleteBatch (pgPool, collection) {
  ...
  try {
    ...
    const data = values.map(doc => Schema.escapeText(doc._id.toString()))
