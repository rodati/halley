...
async function replicateOplogDeletions (specs, pgPool, mongoConnection, concurrency) {
  ...
  for (const spec of specs) {
    ...
    if (spec.keys.incrementalReplicationKey) {
      const max = sql.query(pgClient, `SELECT MAX(${spec.keys.incrementalReplicationKey}) FROM ${spec.target.table}`)
...
async function deleteBatch (operations, pgPool) {
  ...
  await sql.query(pgClient, `CREATE TEMPORARY TABLE "${name}_del_temp" ("${key.name}" TEXT, PRIMARY KEY ("${key.name}"))`)
