...
function * transformValues (spec, doc) {
  ...
  for (const column of columns) {
    ...
    if (source instanceof ObjectId) {
      ...
      value = null
    } else if (typeof source === 'number' && (column.type === 'smallint' || column.type === 'integer' || column.type === 'bigint')) {
      value = Math.trunc(source)
    } else {
      value = source
    }

    yield escapeUnicodeNull(value)
  ...
  if (extraProps) {
    yield JSON.stringify(_.omit(escapeUnicodeNull(doc), omittedFromExtraProps), transformObjectValue)
  }
}

function transformObjectValue (key, value) {
...
function escapeUnicodeNull (target) {
...
function traverse (object) {
  if (typeof object !== 'object') {
    throw new Error('Expected an object to traverse')
  }
  const level = Object.entries(object)
  const result = {}
  for (const [property, value]  of level) {
    if (typeof value === 'string') {
      result[property] = value.replace(/[\\u0000]/g, ' ')
    } else if (typeof value === 'object') {
      result[property] = traverse(value)
    } else {
      continue
    }
  }
