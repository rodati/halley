...
/**
 * Copied from https://github.com/paulpdaniels/rx-node-vx/blob/master/src/writeToStream.js
 */
...
function drainStream (paused, trigger) {
  return source => paused
    // Buffer this stream while we wait to become un-paused
...
function writeToStream (source, stream, encoding) {
  ...
  // : WARNING about this pull request :

  // Making "source" (an observable wrapping a stream) hot in line 32 of
  // lib/rx-node/writeToStream produces an unexpected result while executing
  // the postgres COPY command using copyFrom from node-pg-copy-streams. In
  // some collections, only some of the documents are copied, even when the
  // data seems to be written ok to the copy stream.
