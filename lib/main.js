...
module.exports = async function main (options) {
  ...
  if (process.env.sentryUri) {
    winston.add(sentryTransport('error', process.env.sentryUri))

    // if it's needed to log warnings in sentry:
    // winston.add(sentryTransport('warn'))
  }
  ...
  if (options.incrementalImport) {
    ...
    await replicateOplogDeletions(specs, pgPool, mongoConnection, options.concurrency, del)
  }
