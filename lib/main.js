...
module.exports = async function main (options) {
  ...
  const specsValues = _.values(specs)
  ...
  if (options.incrementalImport) {
    await replicateOplogDeletions(specsValues, pgPool, mongoConnection, options.concurrency)
  }

  // import
  await importCollections(mongoConnection, pgPool, specsValues, options.incrementalImport)
