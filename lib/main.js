...
const sentryTransport = require('./utils/sentryTransport')
...
module.exports = async function main (options) {
  ...
  if (process.env.sentryDsn) {
    ...
    winston.info(`[Sentry] Connecting to ${sentryDsn}`)
    winston.add(sentryTransport('error', sentryDsn))
    // if it's needed to log warnings in sentry:
    // winston.add(sentryTransport('warn', sentryDsn))
  }
  ...
  // import
  await importCollections(mongoConnection, pgPool, values(specs), options.incrementalImport)
  ...
  async function syncObject (spec, selector) {
    ...
    if (obj) {
      ...
      winston.warn(`TODO: sync delete on ${spec.ns}`, JSON.stringify(selector))
    }
  }
...
process.on('unhandledRejection', err => {
  ...
  // note: the process is closed in sentryTransport after the error is logged to sentry
})
