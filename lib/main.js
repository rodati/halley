...
module.exports = async function main (options) {
  ...
  const specs = _.values(await Specs.loadFromFile(options.collections))
  ...
  winston.info(`Connections established succesfuly...`)
  if (options.incrementalImport) {
    await replicateOplogDeletions(specs, pgPool, mongoConnection, options.concurrency)
  }

  // import
  await importCollections(mongoConnection, pgPool, specs, options.incrementalImport)
