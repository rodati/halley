...
async function importCollection (mongoConnection, pgClient, spec, isIncremental) {
  ...
  if (isIncremental && spec.keys.incrementalReplicationKey) {
    ...
    console.log('lastReplicationKeyValue', lastReplicationKeyValue)
...
async function individualUpsertDocs (spec, docs, pgClient) {
  ...
  const text = `INSERT INTO ${spec.target.table} (${placeholders.join(',')}) ON CONFLICT (id) UPDATE`
...
async function replicateDeletions (spec, pgClient, mongoConnection, lastReplicationKeyValue) {
  const mongoStock = await spec.source.getCollection(mongoConnection)
    .find({ }).toArray()

  const pgStock = await sql.query(pgClient, {
    text: `SELECT * FROM ${spec.target.table}`
  })
  console.log('\n==================\n')
  console.log('lastReplicationKeyValue', lastReplicationKeyValue)
  console.log('\nmongo dates: \n', mongoStock.length)
  // mongoStock.forEach(row => console.log(row.updatedAt))
  console.log('\npg dates:\n', pgStock.rowCount)
  // pgStock.rows.forEach(row => console.log(row.updated_at))
  console.log('\n==================\n')
}
